    ...
    <div className="min-h-screen bg-[var(--bg-page)] pb-24">
      <div className="max-w-3xl mx-auto px-6 pt-28">
        <header className="text-center mb-12">
          <span className="text-xs font-black text-[var(--brand)] tracking-widest">MOVE-IN CHECKLIST</span>
          <h1 className="text-4xl font-[1000] mt-2">입주 직후 체크리스트</h1>
          <p className="text-[var(--text-secondary)] font-bold mt-3">입주 당일 ~ 30일, 권리의 뿌리를 세우세요.</p>
        </header>

        <div className="bg-white rounded-[2.5rem] border border-[var(--border)] p-8 shadow-xl mb-10">
        ...
        {MOVE_IN_CHECKLIST.map((phase) => (
          <section key={phase.id} className="mb-12">
            <h2 className="text-2xl font-black text-[var(--text-primary)] mb-1">{phase.label}</h2>
            <p className="text-sm font-bold text-[var(--text-muted)] mb-6">{phase.subtitle}</p>
            <div className="space-y-4">
              {phase.items.map((item) => (
                <label
                  key={item.id}
                  className="flex gap-4 bg-white p-6 rounded-3xl border-2 border-[var(--border)] cursor-pointer hover:border-[var(--brand)] transition-all shadow-sm"
                >
                  <input
                    type="checkbox"
                    checked={!!done[item.id]}
                    onChange={() => toggle(item.id)}
                    className="h-6 w-6 accent-[var(--brand)] mt-0.5"
                  />
                  <div>
                    <p className="font-black text-[var(--text-primary)]">{item.title}</p>
                    <p className="text-sm text-[var(--text-secondary)] font-medium mt-2 leading-relaxed">{item.desc}</p>
                    {item.tip && (
                      <p className="text-xs font-black text-[var(--brand)] mt-2">{item.tip}</p>
                    )}
                  </div>
                </label>
              ))}
        ...
        <div className="text-center">
          <Link
            href="/lease-timeline"
            className="inline-block px-10 py-4 bg-[var(--brand)] text-[var(--brand-on,#fff)] font-black rounded-2xl hover:scale-105 transition-transform"
          >
            타임라인 보기
          </Link>
        </div>

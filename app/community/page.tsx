...
function CommunityContent() {
  ...
  // 알림 링크(?post=)로 들어온 경우: 해당 글이 있는 페이지로 이동 후 펼침
  useEffect(() => {
    if (!postFromUrl || filteredPosts.length === 0) return;

    const index = filteredPosts.findIndex((p) => p.id === postFromUrl);
    if (index < 0) return;

    const targetPage = Math.floor(index / POSTS_PER_PAGE) + 1;
    if (currentPage !== targetPage) {
      setCurrentPage(targetPage);
      return;
    }

    setActivePostId(postFromUrl);
    requestAnimationFrame(() => {
      document
        .getElementById(`post-${postFromUrl}`)
        ?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    });
  }, [postFromUrl, filteredPosts, currentPage]);
  ...
  const handlePostAccordion = async (postId: string, currentViews: number) => {
    ...
    setPosts((prev) =>
      prev.map((p: any) => (p.id === postId ? { ...p, view_count: nextViews } : p))
    );
  };
      ...
      <div className="w-full max-w-2xl space-y-6">
        <div className="flex justify-between items-end w-full pb-2">
          ...
          <div className="flex bg-white border border-slate-200 rounded-xl p-1 shadow-sm">
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-[11px] font-black ${sortBy === 'latest' ? 'bg-slate-900 text-white' : 'text-slate-400'}`}
            >
              최신순
            </button>
            <button
              onClick={() => setSortBy('popular')}
              className={`px-4 py-1.5 rounded-lg text-[11px] font-black ${sortBy === 'popular' ? 'bg-slate-900 text-white' : 'text-slate-400'}`}
            >
              인기순
            </button>
          </div>
        ...
        <div className="flex bg-white border border-slate-200/80 rounded-2xl p-1.5 shadow-sm gap-1 w-full">
          <button
            ...
            className={`flex-1 py-3 text-xs font-black rounded-xl transition-all ${filterCategory === 'all' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
          >
            🌏 전체보기
          </button>
          <button
            onClick={() => setFilterCategory('주택')}
            className={`flex-1 py-3 text-xs font-black rounded-xl transition-all ${filterCategory === '주택' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
          >
            🏠 주택 게시판
          </button>
          <button
            onClick={() => setFilterCategory('상가')}
            className={`flex-1 py-3 text-xs font-black rounded-xl transition-all ${filterCategory === '상가' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 hover:text-slate-700'}`}
          >
            🛍️ 상가 게시판
          </button>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-4 pl-1">
            ...
            <button
              onClick={() => setNewPostCategory('주택')}
              className={`px-4 py-1.5 rounded-xl text-xs font-black border transition-all ${newPostCategory === '주택' ? 'bg-blue-50 border-blue-500 text-blue-600' : 'bg-slate-50 border-transparent text-slate-400'}`}
            >
              🏠 주택 관련
            </button>
            <button
              onClick={() => setNewPostCategory('상가')}
              className={`px-4 py-1.5 rounded-xl text-xs font-black border transition-all ${newPostCategory === '상가' ? 'bg-orange-50 border-orange-500 text-orange-600' : 'bg-slate-50 border-transparent text-slate-400'}`}
            >
              🛍️ 상가 관련
            </button>
          </div>
          <input
            type="text"
            placeholder="고민이나 정보를 공유해 주세요"
            value={newPostTitle}
            onChange={(e) => setNewPostTitle(e.target.value)}
            className="w-full p-4 bg-slate-50 rounded-2xl border-none outline-none font-black text-slate-900 mb-3 placeholder:text-slate-300"
          />
          <textarea
            ...
            className="w-full p-4 bg-slate-50 rounded-2xl border-none outline-none font-bold text-slate-700 text-sm resize-none mb-3 placeholder:text-slate-300"
          />
          <button
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-3 shrink-0 self-start sm:self-center">
                    <span
                      className={`px-2 py-0.5 rounded-md text-[9px] font-black border ${post.category === '상가' ? 'bg-orange-50 text-orange-500 border-orange-100' : 'bg-blue-50 text-blue-500 border-blue-100'}`}
                    >
                      {post.category === '상가' ? '🛍️ 상가' : '🏠 주택'}
                    </span>
                    <span className="text-[10px] font-black text-slate-700 bg-slate-50 px-2.5 py-1 rounded-full">
                ...
                {activePostId === post.id && (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-6 border-t border-b border-slate-50 py-3">
                      ...
                      >
                        <span
                          className={`text-lg transition-transform group-hover:scale-125 ${post.is_liked ? 'grayscale-0' : 'grayscale'}`}
                        >
                          👍
                        </span>
                        <span
                          className={`text-xs font-black ${post.is_liked ? 'text-blue-600' : 'text-slate-400'}`}
                        >
                          {post.like_count} 명 추천
                        </span>
                      </button>
                      <div className="flex items-center gap-1.5">
                        <span className="text-lg">💬</span>
                        <span className="text-xs font-black text-slate-400">
                          답글 {post.comments?.length || 0}개
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4 bg-slate-50 p-6 rounded-3xl">
                      {post.comments?.length === 0 ? (
                        ...
                        post.comments?.map((comment: any) => (
                          <div
                            key={comment.id}
                            className="flex justify-between items-start border-b border-white pb-3 last:border-0 last:pb-0"
                          >
                      ...
                      <div className="flex gap-2 mt-4 pt-1">
                        <input
                          ...
                          className="flex-1 p-3 bg-white rounded-2xl border-none outline-none text-sm font-bold shadow-sm focus:ring-2 focus:ring-blue-500"
                        />
                        <button
                          onClick={() => addComment(post)}
                          className="px-5 py-3 bg-slate-900 text-white rounded-2xl text-xs font-black hover:bg-black"
                        >
                          등록
                        </button>
                      </div>
          ...
          <div className="flex flex-col items-center gap-3 pt-2 pb-6">
            ...
            <div className="flex items-center gap-2 flex-wrap justify-center">
              <button
                ...
                className="px-3 py-2 rounded-xl text-xs font-black border border-slate-200 bg-white disabled:opacity-40 hover:border-slate-400"
              >
                이전
              </button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  ...
                  className={`min-w-[36px] px-3 py-2 rounded-xl text-xs font-black border transition-all ${
                    safePage === page

/** Supabase SQL Editor에 붙여넣어 1회 실행 (문의 답변 + 알림) */
export const FEEDBACK_MIGRATION_SQL = `-- 문의·요청 답변 + 알림
...
begin
  for admin_id in select p.id from public.profiles p where p.is_admin = true loop
    if admin_id = new.author_id then continue; end if;
...
returns trigger language plpgsql security definer set search_path = public as $$
declare replier uuid;
begin
  if new.admin_reply is null or char_length(trim(new.admin_reply)) = 0 then return new; end if;
  if old.admin_reply is not distinct from new.admin_reply then return new; end if;
  replier := coalesce(new.replied_by, auth.uid());
  if new.author_id is null or new.author_id = replier then return new; end if;
  insert into public.notifications (user_id, actor_id, post_id, feedback_id, type, is_read)
...
returns table (
  id uuid, author_id uuid, title text, content text, is_public boolean,
  content_masked boolean, admin_reply text, replied_at timestamptz, created_at timestamptz
)
language sql stable security definer set search_path = public as $$
  select fr.id, fr.author_id, fr.title,
    case when fr.is_public then fr.content
      when auth.uid() is not null and fr.author_id = auth.uid() then fr.content
      when public.is_app_admin() then fr.content
      else '비공개 글입니다. 작성자와 운영자만 내용을 확인할 수 있어요.' end as content,
    fr.is_public,
    (not fr.is_public and not (auth.uid() is not null and fr.author_id = auth.uid()) and not public.is_app_admin()) as content_masked,
    case when fr.admin_reply is null or char_length(trim(fr.admin_reply)) = 0 then null
      when fr.is_public then fr.admin_reply
      when auth.uid() is not null and fr.author_id = auth.uid() then fr.admin_reply
      when public.is_app_admin() then fr.admin_reply else null end as admin_reply,
    fr.replied_at, fr.created_at
  from public.feedback_requests fr order by fr.created_at desc;
$$;
...
export function isFeedbackSchemaError(message: string): boolean {
  return /admin_reply|replied_at|replied_by|schema cache/i.test(message);
}
